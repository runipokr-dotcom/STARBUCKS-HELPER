...
3. ChatGPT 또는 Claude와 실행 암호를 입력하고 **실행하기**를 누른다.
4. 결과의 **결과 복사 / 다시 생성 / 다른 모델로 실행**을 사용한다. 재생성 및 모델 변경은 마지막 성공 결과와 동일한 입력을 사용한다. 입력을 수정해 실행하려면 위쪽 **실행하기**를 누른다.
...
- 이번 작업은 로컬 수정/검수까지 진행했으며 commit/push/운영 배포, 실제 유료 호출, 환경변수 등록은 수행하지 않았다.
